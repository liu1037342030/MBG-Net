...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            # print(sample['image'].shape)   预处理后的shape(12,3,512,512)
            # print(sample['map'].shape)                  (12,2,512,512)
            imageS = sample['image'].cuda()
            target_map = sample['map'].cuda()
            target_boundary = sample['boundary'].cuda()
            #预测出来的mask和边界结果  oS-mask   boundaryS-边界
            #训练生成器参数
            ...
            #根据权重系数将mask和边界损失相加  这里的bd_weight为1  loss_seg代表总的损失
            ...
            if self.epoch > self.warmup_epoch:
                ...
                #这里算出总的对抗网络的损失  损失等于边界损失加mask损失再乘以协同系数 coefficient=0.01
